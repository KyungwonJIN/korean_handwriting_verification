...
file_name = 'mm_up'
weight_name = 'class5_205-0.0658_modify'

# PRED_COODS_FILE = "siamese_model/parrot_siamese.csv" #예측한 좌표값 저장파일이름
PRED_COODS_FILE = './mm_predict/' + file_name + '_mod' + '.csv'
PRED_IMAGE_FILE = './mm_predict/'
...
hdf5_path = './mm_hdf5_test_name/' + file_name + '.hdf5'
# open the hdf5 file
...
model = load_model('./mm_modify_weight/' + weight_name + '.hdf5')  #저장된 모델
model.summary()
...
predicted_binary = (predicted_pre > -4).astype(np.int)
