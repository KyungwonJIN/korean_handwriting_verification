...
def get_resnet(input_shape, num_outputs):
    ...
    # convert_conv0 =_conv_single(filters=1, kernel_size=(1, 1), strides=(1, 1))(input)
...
#     conv1 = _conv_bn_relu(filters=64, kernel_size=(7, 7), strides=(1, 1))(input)
#     pool1 = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding="same")(conv1)
#     block1 = _residual_block(basic_block_fn, filters=64, repetitions=2, is_first_layer=True, reduce_output=False)(pool1)
#     block2 = _residual_block(basic_block_fn, filters=128, repetitions=2, is_first_layer=False, reduce_output=True)(block1)
#     block3 = _residual_block(basic_block_fn, filters=256, repetitions=2, is_first_layer=False, reduce_output=True)(block2)
#     final_block = _residual_block(basic_block_fn, filters=512, repetitions=2, is_first_layer=False, reduce_output=True)(block3)
    ...
    # conv1 = _conv_bn_relu(filters=64, kernel_size=(7, 7), strides=(1, 1))(input)
    # pool1 = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding="same")(conv1)
    # block1 = _residual_block(bottleneck_fn, filters=64, repetitions=3, is_first_layer=True, reduce_output=False)(pool1)
    # block2 = _residual_block(bottleneck_fn, filters=128, repetitions=4, is_first_layer=False, reduce_output=True)(block1)
    # block3 = _residual_block(bottleneck_fn, filters=256, repetitions=6, is_first_layer=False, reduce_output=True)(block2)
    # final_block = _residual_block(bottleneck_fn, filters=512, repetitions=3, is_first_layer=False, reduce_output=True)(block3)

    # Last activation
    block = _bn_relu(final_block)

    # Classifier bl0 of size 512ock
...
def get_denseNet(input_shape, num_outputs):
    ...
    model = DenseNet121(input_tensor=input, include_top=True, weights='imagenet', pooling='max')
    ...
    model_output_start = model.output
    # flatten = Flatten()(model_output_start)
    dense = Dense(units=num_outputs, kernel_initializer="he_normal", kernel_regularizer=l2(2e-4), activation="sigmoid")(model_output_start)
